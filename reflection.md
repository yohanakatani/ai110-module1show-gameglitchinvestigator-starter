...
| Clicked "New Game" after winning a game | Full reset: blank input field, attempts back to 1, fresh secret, cleared score display | Partial reset — status, history, and attempts clear correctly, but the text input widget retains the last guess and the score persists from the prior game | No error; previous guess still visible in the input box after restart — this misbehavior remains unfixed |
